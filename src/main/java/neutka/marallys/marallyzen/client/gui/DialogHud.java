    ...
    public void renderInWorld(PoseStack poseStack, Camera camera, float partialTick) {
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            if (backgroundAlpha > 0.0f) {
                ...
                // Use same color as proximity overlay: black with ~47% opacity (alpha * 120)
            ...
            int textColor;
            if (isSelected) {
                // Selected text uses NPC name color with appear alpha
                textColor = (alpha << 24) | (npcNameColor & 0xFFFFFF);
            } else {
