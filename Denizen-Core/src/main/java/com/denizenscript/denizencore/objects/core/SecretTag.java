    ...
    public static void load() {
        ...
        if (fileContent != null) {
            ...
            try {
                ...
                // Try to extract detailed error information from YAML exceptions
                String errorMsg = "Error occurred while loading secrets file: " + e.getMessage();
                try {
                    // Try to get MarkedYAMLException details using reflection
                    if (e.getClass().getName().equals("org.yaml.snakeyaml.error.MarkedYAMLException")) {
                        java.lang.reflect.Method getProblemMark = e.getClass().getMethod("getProblemMark");
                        Object mark = getProblemMark.invoke(e);
                        if (mark != null) {
                            java.lang.reflect.Method getLine = mark.getClass().getMethod("getLine");
                            java.lang.reflect.Method getColumn = mark.getClass().getMethod("getColumn");
                            java.lang.reflect.Method getProblem = e.getClass().getMethod("getProblem");
                            int line = (Integer) getLine.invoke(mark);
                            int column = (Integer) getColumn.invoke(mark);
                            String problem = (String) getProblem.invoke(e);
                            errorMsg = "Error occurred while loading secrets file, on line " + (line + 1) + ", column " + (column + 1) + ": " + problem;
                        }
                    }
                }
                catch (Exception reflectionException) {
                    // If reflection fails, just use the basic error message
                }
                Debug.echoError(errorMsg);
            }
        }
        else {
